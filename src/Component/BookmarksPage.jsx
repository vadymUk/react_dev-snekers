...
import snaker from "./img/snaker.png";

export default function BookmarksPage({ marks, setMarks }) {
    const { counting, add, setAdd } = useContext(CartContext);

    const onBookmarks = (obj) => {
        if (add.find((item) => item.id === obj.id)) {
            setMarks(marks.filter((item) => item.id !== obj.id));
        } else {
            setMarks(marks.filter((item) => item.id !== obj.id));
    ...
    return (
        <>
            ...
            <div className='container'>
                <Link to='/'>
                    <h2 className='shoping__title'>Моі закладки</h2>
                </Link>
            </div>
            <div className='container'>
